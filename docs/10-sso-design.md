...
進階客製走 `XITTO_AUTH_MODULE`（自訂 `mapClaims`/`authorize`），仍限 OAuth2。
...
- **S2**：`roleStore`（`roles.json` 讀寫）+ `roleOf` 判定 + `/v1/admins` API。
- **S3**：`oauth2Auth`（discovery / 顯式端點 / JWKS / PKCE+state+nonce / cookie session）+ `startServer` 環境變數掛載。
- **S4**：前端登入導向、會議室 principal 身份、`docs/sso` 使用指南（中英）。
