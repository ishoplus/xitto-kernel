...
![xitto 任務台 — 說一句話、看它做事、拿成品](https://raw.githubusercontent.com/ishoplus/xitto-kernel/main/assets/wishboard.png)

> 🪄 **任務台** 網頁（其中一個前端）：打一句話 → 背景自動跑 → 看得到即時過程（階段、步驟、彩色 diff），做完直接拿成品。

## 一句話

> **kernel 提供「怎麼跑一個 agent」，DomainPack 提供「這個 agent 會什麼、守什麼」。**

## 這給誰用——以及何時該改用 Claude Code

這**不是**要在寫程式上贏過 Claude Code。在它的主場——工程師用 Claude 做編碼任務——Claude Code 更強、更成熟。**那是你的情況，就用它。**

**當 Claude Code 結構上服務不到你時，才用 xitto-kernel：**
- **必須用非 Claude 模型**——大規模成本、地區/合規（如資料主權）、或離線/地端本地模型。Claude Code 只跑 Claude；kernel 跑 MiniMax / DeepSeek / OpenAI / 本地 OSS / 自訂。
- **你要做「自己的」agent 產品**——自家介面、領域、後端。kernel 是讓你 `import` 來蓋產品的庫，不是直接採用的成品。
- **使用者不是工程師**——📋 任務台讓非技術使用者打一句話、收成品（PDF/DOCX…），而非終端/IDE。
- **領域不是編碼**——專利交底、查資料、知識庫、文件產出，各帶自己的驗收閘門（完成定義）。

kernel 的優勢不是原始能力，而是**觸及範圍**（哪些模型、哪些人、哪個領域、哪種部署）＋ 可驗證的「完成」契約 ＋ 越用越懂你專案的累積脈絡。
...
**本地就地模式(像 Claude Code 改你選的真實資料夾)**:`XITTO_SERVER_LOCAL=1` 時,網頁多一個「**📁 選資料夾**」鈕——**用點的**從家目錄瀏覽進你的真實資料夾並選定(不用打路徑;瀏覽器拿不到絕對路徑,所以由 local server 端列資料夾),或「新專案」直接貼絕對路徑也行。任務就**就地改那個資料夾的檔**(不另開隔離副本),工作台列的也是它。這把「任務台(隔離,服務非技術使用者)」和「Claude Code(就地,改你現有的 codebase)」兩個模型打通:**本機自用想就地 → 給路徑;隔離/託管 → 給名稱**。**安全**:只在 `local` 模式才認絕對路徑;**託管模式收到絕對路徑會被消毒成管理空間,不會逃逸到主機任意路徑**。

**重啟後歷史還在(持久化)**:任務清單落地 `.xitto-server/tasks/`、對話 session 落地 `.xitto-server/sessions/`,啟動時載回——所以**重啟後歷史成品自動顯示、舊成品仍能「繼續/調整」**(對話脈絡也在)。重啟時還在跑/待答的任務會標「已中斷(重啟)」。對標 Claude Code「對話自動落地」,但任務台是**自動顯示歷史**(成品清單),而非 Claude Code 的明確 `--resume`。
