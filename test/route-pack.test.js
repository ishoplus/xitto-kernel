...
test('heuristicPack：強訊號 → 對應領域；一般任務 → null（交給 general）', () => {
  ...
  assert.equal(heuristicPack('做一份專案簡介的 PDF 報告'), 'docgen');
  assert.equal(heuristicPack('幫我把這份資料匯出成 csv'), 'docgen');
  assert.equal(heuristicPack('做一份 2026 世界盃的深度研究報告，多來源查證'), 'deep-research', 'docgen 不該搶走 deep-research');
