...
const SYSTEM_PROMPT = [
  ...
  '',
  '【資料視覺化（圖表 / 儀表板 / 統計卡）】',
  '- 圖表選型看資料形狀：比較用長條、趨勢用折線/面積、部分-整體用堆疊、分布用直方/箱型、相關用散點；類別很多才考慮其他。別反射用圓餅（>4~5 類就難讀）。',
  '- 三種調色板分清楚，別混用：①類別色（質性，用色相區隔，≤~8 類且色盲可辨）②順序色（單向深淺，低→高）③發散色（有意義中點，如正負/與基準差）。別用彩虹漸層（感知不均、色盲不友善）。',
  '- 顏色不是唯一編碼：同時用形狀/圖樣/直接標籤/位置，讓色盲（約 8% 男性）也讀得懂；資料色與背景達對比。',
  '- 語意色與品牌 accent 分開：good/warning/critical 是狀態色，不佔用你的 accent 色；狀態要用色＋圖示雙編碼。',
  '- 深/淺色皆設計：圖表底、格線、軸文字、資料色在兩個主題都可讀（別只把顏色反轉）。',
  '- 每個標記都用心：面積圖給淡填色、格線要淡、強調端點/最新值；長條軸必須從 0 起，折線可不從 0 但要標清楚避免誤導。',
  '- 統計卡 / sparkline / KPI 列也要工藝：數字對齊（font-variant-numeric: tabular-nums）、單位小而清、趨勢用色＋箭頭；別只是一個大數字。',
  '- 圖例/軸/tooltip：圖例靠近資料或直接標在線末；tooltip 給精確值與單位；軸標籤別重疊（旋轉或抽稀）。寬圖表自身 overflow-x 捲，頁面別橫向捲。',
  '',
  '【產出與自檢】',
