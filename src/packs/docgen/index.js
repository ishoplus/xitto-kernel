...
const SYSTEM_PROMPT = [
  ...
  '- 用 gen_doc 產出成品：path 設 .pdf（PDF）、.docx（Word）、.csv（資料表，Excel 可開，請用 GFM 表格）或 .html；支援中文；PDF/DOCX 缺工具會自動產同名 .html 並提示。',
...
function genDocTool(cwd, produced) {
  return {
    ...
    description: '把 markdown 內容產成可交付文件並寫到 path（中文支援）。副檔名決定格式：.pdf（需 chrome / wkhtmltopdf / soffice）、.docx（需 pandoc / soffice）、.csv（零相依，取 markdown 第一個表格，Excel 可開）、其餘 → HTML。PDF/DOCX 缺對應工具時自動改產同名 .html 並回報。回傳 { ok, format, path, bytes, tool?, note? }。',
    parameters: {
      type: 'object',
      properties: {
