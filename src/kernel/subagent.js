...
async function runSub({ model, getApiKey, tools, task, streamFn, onEvent }) {
  const { Agent } = await import('./agent-loop.js');
  const sf = streamFn || (await import('./provider.js')).defaultStreamFn();
  const sub = new Agent({
    initialState: { systemPrompt: SUB_PROMPT, model, tools, thinkingLevel: model.reasoning ? 'low' : 'off' },
    getApiKey,
    streamFn: sf,
 ...
 */
export function createSpawnTool({ getModel, getApiKey, getReadOnlyTools, getStreamFn }) {
  return {
    ...
    execute: async (_id, { task }, _signal, onPartial) => {
      ...
      const onEvent = typeof onPartial === 'function'
        ...
        : null;
      try { return txt(await runSub({ model, getApiKey, tools: getReadOnlyTools(), task, streamFn: getStreamFn?.(), onEvent })); }
 ...
 */
export function createMapTool({ getModel, getApiKey, getReadOnlyTools, getStreamFn }) {
  return {
    name: 'spawn_agents', label: '平行子 agent', readOnly: true,
    description: '派「多個」唯讀子 agent 平行各做一件聚焦調查（讀檔/搜尋/分析），回傳每項結論。'
      + `適合對很多項目做同一種需要理解的處理（如掃多個檔/模組/來源），量大序列太慢時。`
      + `一次最多 ${MAP_MAX} 項、同時 ${MAP_CONCURRENCY} 個在跑。子 agent 皆唯讀、不能改檔或跑命令。`,
    parameters: {
      type: 'object',
      properties: { tasks: { type: 'array', items: { type: 'string' }, description: '要平行調查的任務清單，每項一個子 agent（具體、可獨立完成）' } },
      required: ['tasks'],
    },
    execute: async (_id, { tasks }, _signal, onPartial) => {
      const model = getModel?.();
      if (!model || !getApiKey) return txt({ error: '無 model/apiKey，無法派子 agent' });
      ...
      const results = await pool(list, MAP_CONCURRENCY, async (task, i) => {
        ...
        try {
          const text = await runSub({ model, getApiKey, tools, task, streamFn });
